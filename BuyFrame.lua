...
local SummarySort = {sort = 'ItemSummary', flipped = false, justFlipped = false, sorted = false}
local DetailSort = {sort = 'BuyoutEach', flipped = false, justFlipped = false, sorted = false}
...
function AuctionLite:SetBuyData(results)
    ...
    if BuyMode == BUY_MODE_DEALS or BuyMode == BUY_MODE_SCAN then
...
function AuctionLite:SetScanData(results)
    ...
    for link, result in pairs(results) do
        ...
        if min > 0 and hist ~= nil and min < hist.price - (10000 * self.db.profile.minProfit) and min < hist.price *
            (1 - self.db.profile.minDiscount) and hist.listings / hist.scans > 1.2 then
...
function AuctionLite:FindMyListings(listings, list, name, onlyUndercut)
    ...
    table.sort(listingsCopy, function(a, b)
                ...
                for _, count in ipairs(indices) do
                    ...
                    if newCount < maxCount then
                        ...
                        if newInfo == nil or newPrice < newInfo.price then
                            ...
                            results[newCount] = {price = newPrice, listing = listing, info = info}
                        end
...
function AuctionLite:CreateOrder(isBuyout, requested)
    if DetailLink ~= nil then
        ...
        local order = {
            list = {},
            name = self:SplitLink(DetailLink),
            price = 0,
            spent = 0,
            count = 0,
            isBuyout = isBuyout,
...
function AuctionLite:EndPurchase(cancelled)
    ...
    if not cancelled or order.listingsBought > 0 then
        if order.isBuyout then
            self:Print(L['Bought %dx %s (%d |4listing:listings; at %s).']:format(order.itemsBought, order.name,
                                                                                 order.listingsBought,
                                                                                 self:PrintMoney(order.spent)))
        else
            self:Print(L['Bid on %dx %s (%d |4listing:listings; at %s).']:format(order.itemsBought, order.name,
                                                                                 order.listingsBought,
                                                                                 self:PrintMoney(order.spent)))
        end

        if order.itemsNotFound > 0 then
            self:Print(L['Note: %d |4listing:listings; of %d |4item was:items were; not purchased.']:format(
                           order.listingsNotFound, order.itemsNotFound))
        end
    end
...
function AuctionLite:MultiScan(items)
    ...
    if MultiScanCurrent ~= nil then
        ...
        local query = {
            ...
            exact = true,
            update = function(pct)
                AuctionLite:UpdateProgressMulti(pct)
            end,
            finish = function(data)
                AuctionLite:SetMultiScanData(data)
            end,
        }
...
function AuctionLite:ApplyDetailSort()
    ...
    elseif info.sort == 'BidEach' then
        cmp = function(a, b)
            return a.bid / a.count < b.bid / b.count
        end
...
function AuctionLite:ApplySummarySort()
    ...
    elseif info.sort == 'Listings' then
        cmp = function(a, b)
            return a.listingsAll < b.listingsAll
        end
    elseif info.sort == 'Items' then
        cmp = function(a, b)
            return a.itemsAll < b.itemsAll
        end
    ...
    elseif info.sort == 'Historical' then
        if BuyMode == BUY_MODE_DEALS or BuyMode == BUY_MODE_SCAN then
            ...
            cmp = function(a, b)
                return a.histPrice < b.histPrice
            end
        end
    else
        assert(false)
            ...
            local menuList = {
                {
                    ...
                    func = function()
                        AuctionLite:CancelItems(true, link)
                    end,
                }, {
                    text = L['Cancel All Auctions'],
                    func = function()
                        AuctionLite:CancelItems(false, link)
                    end,
                },
            }
...
function AuctionLite:BuyAdvancedButton_OnClick()
    ...
    BuyAdvancedDropDown.initialize = function(menu, level)
        ...
        if level == 1 then
            ...
            info.func = function()
                AuctionLite:AuctionFrameBuy_Deals()
            end
...
function AuctionLite:AuctionFrameBuy_Search()
    ...
    local query = {
        ...
        update = function(pct)
            AuctionLite:UpdateProgressSearch(pct)
        end,
        finish = function(data)
            AuctionLite:SetBuyData(data)
        end,
    }
...
function AuctionLite:AuctionFrameBuy_UpdateExpand()
    ...
    if order ~= nil then
        if order.resell == nil then
            ...
            BuyExpand3Text:SetText(L['Net cost for %d:']:format(order.count - order.resell))
            ...
            BuyExpand4Text:SetText(L['Historical price for %d:']:format(order.count - order.resell))
...
function AuctionLite:AuctionFrameBuy_UpdateDetail()
    ...
    local _, _, _, _, enchant, jewel1, jewel2, jewel3, jewel4 = self:SplitLink(DetailLink)
...
function AuctionLite:AuctionFrameBuy_UpdateSummary()
    ...
    for i = 1, displaySize do
        ...
        if item ~= nil then
            ...
            if enchant ~= 0 or jewel1 ~= 0 or jewel2 ~= 0 or jewel3 ~= 0 or jewel4 ~= 0 then
                plusText:SetPoint('LEFT', nameText, 'LEFT', nameText:GetStringWidth(), 0)
                plusText:Show()
            else
                plusText:Hide()
...
function AuctionLite:ClearBuyFrame(partial)
    ...
    if not partial then
        ...
        SummarySort = {sort = 'ItemSummary', flipped = false, justFlipped = false, sorted = false}

        DetailSort = {sort = 'BuyoutEach', flipped = false, justFlipped = false, sorted = false}
    end
