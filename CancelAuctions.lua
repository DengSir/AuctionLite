...
function AuctionLite:CancelAuctions(targets)
    ...
    for i = 1, batch do
        ...
        for _, target in ipairs(targets) do
            if not target.found and self:MatchListing(target.name, target, listing) then

                target.found = true

                local item = {index = i, target = target, hasBid = (listing.bidAmount > 0)}
...
function AuctionLite:FinishCancel(cancel, cancelBid)
    ...
    table.sort(cancel, function(a, b)
