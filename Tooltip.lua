...
function AuctionLite:SetTooltipMoney(tooltip, label, money)
    ...
    if moneyFrame == nil then
        moneyFrame = CreateFrame('Frame', moneyName, tooltip, 'TooltipMoneyFrameTemplate')
    ...
    if option ~= 'c_no' then
        ...
        if price ~= nil and price > 0 then
            ...
            if self.db.profile.coinTooltips then
                ...
                if count2 ~= nil then
                    priceInfo = priceInfo .. ' |cffffffff-|r ' .. self:PrintMoney(price * count2)
                end
...
function AuctionLite:AddTooltipData(tooltip, link, count1, count2)
    if link ~= nil and count1 ~= nil and
        (self.db.profile.showVendor ~= 'c_no' or self.db.profile.showDisenchant ~= 'c_no' or self.db.profile.showAuction ~=
            'c_no') then
        ...
        if (stackPrice and IsShiftKeyDown()) or (not stackPrice and not IsShiftKeyDown()) then
            count1 = 1
            count2 = nil
        end
        ...
        self:AddTooltipLine(tooltip, self.db.profile.showVendor, function(link)
            return vendor
        end, '|cffffd000' .. L['Vendor'] .. '|r' .. suffix, link, count1, count2)

        self:AddTooltipLine(tooltip, self.db.profile.showDisenchant, function(link)
            return AuctionLite:GetDisenchantValue(link)
        end, '|cffffd000' .. L['Disenchant'] .. '|r' .. suffix, link, count1, count2)

        self:AddTooltipLine(tooltip, self.db.profile.showAuction, function(link)
            return AuctionLite:GetAuctionValue(link)
        end, '|cffffd000' .. L['Auction'] .. '|r' .. suffix, link, count1, count2)
...
function AuctionLite:InventoryTooltip(tooltip, unit, slot)
    if tooltip:NumLines() > 0 and not (20 <= slot and slot <= 23) and -- skip inventory bags
