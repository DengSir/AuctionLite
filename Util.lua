...
function AuctionLite:SplitLink(link)
    ...
    return name, color, tonumber(id) or 0, tonumber(suffix) or 0, tonumber(enchant) or 0, tonumber(jewel1) or 0,
           tonumber(jewel2) or 0, tonumber(jewel3) or 0, tonumber(jewel4) or 0, tonumber(unique) or 0
end

-- Zero out the uniqueId field from an item link.
function AuctionLite:RemoveUniqueId(link)
    if link ~= nil then
        return link:gsub( -- Fields: itemID, enchant, gem1, gem2, gem3, gem4, suffixID.
        '(Hitem:%-?%d*:%-?%d*:%-?%d*:%-?%d*:%-?%d*:%-?%d*:%-?%d*)' .. -- Fields: uniqueID, level.
        ':%-?%d*:%-?%d*', -- Set uniqueID and level to 0.
        '%1::')
    else
        return nil
    end
...
function AuctionLite:RemoveSuffix(link)
    if link ~= nil then
        ...
        return link:gsub( -- Fields: itemID, enchant, gem1, gem2, gem3, gem4.
        '(Hitem:%-?%d*:%-?%d*:%-?%d*:%-?%d*:%-?%d*:%-?%d*)' .. -- Fields: suffixID.
        ':%-?%d*', -- Set suffixID to 0.
...
function AuctionLite:GetListing(kind, i)
    ...
    local name, texture, count, quality, canUse, level, levelColHeader, minBid, minIncrement, buyout, bidAmount,
...
function AuctionLite:MatchListing(targetName, target, listing)
    return
        targetName == listing.name and target.count == listing.count and target.bid == listing.bid and target.buyout ==
            listing.buyout and (target.owner == nil or listing.owner == nil or target.owner == listing.owner)
end
...
function AuctionLite:MatchPages(data, page1, page2)
    ...
    for i = 1, NUM_AUCTION_ITEMS_PER_PAGE do
        ...
        if listing1 == nil or listing2 == nil or not self:MatchListing(listing1.name, listing1, listing2) then
            return false
        end
    end
