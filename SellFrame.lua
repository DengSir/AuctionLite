...
local SellSort = {sort = 'BuyoutEach', flipped = false, justFlipped = false, sorted = false}
...
MoneyTypeInfo['AUCTIONLITE_DEPOSIT'] = {
    UpdateFunc = function()
        return AuctionLite:CalculateDeposit()
    end,
    collapse = 1,
}
...
function AuctionLite:GeneratePrice(value, allowUndercut)
    ...
    local bid = generate(value, bidUndercut, bidUndercutFixed, granularity)

    local buyout = generate(value, buyoutUndercut, buyoutUndercutFixed, granularity)
...
function AuctionLite:ShowPriceData(itemLink, itemValue, stackSize)
    ...
    if hist ~= nil and hist.scans > 0 and hist.price > 0 then
        self:Print(L['Historical: %s (%d |4listing:listings;/scan, %d |4item:items;/scan)']:format(
                       self:PrintMoney(hist.price * stackSize), math.floor(0.5 + hist.listings / hist.scans),
                       math.floor(0.5 + hist.items / hist.scans)))
        if itemVendor > 0 then
            self:Print(L['Current: %s (%.2fx historical, %.2fx vendor)']:format(self:PrintMoney(stackValue),
                                                                                math.floor(100 * itemValue / hist.price) /
                                                                                    100, math.floor(
                                                                                    100 * itemValue / itemVendor) / 100))
        else
            self:Print(L['Current: %s (%.2fx historical)']:format(self:PrintMoney(stackValue),
                                                                  math.floor(100 * itemValue / hist.price) / 100))
        end
    elseif itemVendor > 0 then
        self:Print(L['Current: %s (%.2fx vendor)']:format(self:PrintMoney(stackValue),
...
function AuctionLite:ValidateAuction()
    local name, _, count, _, _, vendor, link = self:GetAuctionSellItemInfoAndLink()
...
function AuctionLite:ClickAuctionSellItemButton_Hook()
    ...
    if AuctionFrameSell:IsShown() and not self:CreateInProgress() then
        ...
        local name, texture, count, _, _, _, link = self:GetAuctionSellItemInfoAndLink()
        ...
        if name ~= nil then
            ...
            SellStackText:SetText(L['Number of Items |cff808080(max %d)|r']:format(total))
            ...
            if IsControlKeyDown() then
                ...
                local query = {
                    ...
                    finish = function(data, link)
                        AuctionLite:SetSellData(data, link)
                    end,
                }
...
function AuctionLite:ClearSellFrame()
    ...
    SellSort = {sort = 'BuyoutEach', flipped = false, justFlipped = false, sorted = false}
...
function AuctionLite:SetSellData(results, link)
    ...
    if result ~= nil and result.price > 0 then
        ...
        self:SetStatus(L['|cff00ff00Scanned %d listings.|r']:format(result.listings))
    else
        ...
        if hist ~= nil and hist.price > 0 then
            ...
            self:SetStatus(L['|cffff0000Using %.3gx vendor price.|r']:format(mult))
        end
        allowUndercut = false
    end
...
function AuctionLite:SellRememberButton_OnClick(widget)
    ...
    if link ~= nil then
        ...
        local menuList = {{text = L['Saved Item Settings'], isTitle = true}}
...
function AuctionLite:AuctionFrameSell_Update()
    ...
    if SellLink ~= nil then
        name, color, _, _, enchant, jewel1, jewel2, jewel3, jewel4 = self:SplitLink(SellLink)

        showPlus = enchant ~= 0 or jewel1 ~= 0 or jewel2 ~= 0 or jewel3 ~= 0 or jewel4 ~= 0
    end
...
function AuctionLite:SellScrollFrame_OnVerticalScroll(offset)
    FauxScrollFrame_OnVerticalScroll(SellScrollFrame, offset, SellButton1:GetHeight(), function()
        AuctionLite:AuctionFrameSell_Update()
    end)
end
