...
function AuctionLite:QueryUpdate()
    ...
    if canSend and Query ~= nil and Query.state == QUERY_STATE_SEND then
        ...
        if name ~= nil and (not Query.getAll or (Query.page == 0 and Query.listing == nil)) then
            ...
            if Query.getAll then
                if canGetAll then
                    ...
                    Query.getAll = false
                    self:Print(L['|cffffd000[Note]|r ' .. 'Fast auction scans can only be used once every ' ..
    ...
    if Query ~= nil and Query.state == QUERY_STATE_WAIT and Query.time ~= nil and Query.time + QUERY_DELAY < time() then
        Query.time = nil
        self:QueryNewData()
    end
end
...
function AuctionLite:AUCTION_ITEM_LIST_UPDATE()
    ...
    if Query ~= nil and Query.state == QUERY_STATE_WAIT then
        ...
        local duplicate = Query.page > 0 and self:MatchPages(Query.data, Query.page - 1, Query.page)
