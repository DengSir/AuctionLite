...
function AuctionLite:StartAuctions(bid, buyout, time, size, stacks, name, link)
    ...
    if GetAuctionSellItemInfo() ~= nil then
        ...
        self:Print(L['Created %d |4auction:auctions; of %s x%d (%s total).']:format(stacks, name, size,
                                                                                    self:PrintMoney(stacks * buyout)))
    end
end
...
function AuctionLite:CreateAuctionsCore()
    ...
    if not Selling then
        ...
        local name, _, count, _, _, _, link, sellContainer, sellSlot = self:GetAuctionSellItemInfoAndLink()
...
function AuctionLite:CreateAuctions()
    self:StartCoroutine(function()
        AuctionLite:CreateAuctionsCore()
    end)
end
